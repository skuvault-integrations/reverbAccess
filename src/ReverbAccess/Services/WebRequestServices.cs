...
namespace ReverbAccess.Services
{
	internal class WebRequestServices
	{
		...
		public WebRequestServices( ReverbConfig config )
		{
			...
			//this.ResolveHost( string.Concat( config.NativeHost, ReverbCommand.GetOrdersCount.Command ) );
		}
        ...
        {
            T result;
            var request = this.CreateGetServiceGetRequest(string.Concat(this._host, String.Format(command.Command, commandFormatParams), commandParams));
            using (var response = request.GetResponse())
                result = ParseResponse<T>(response);
		...
		private HttpWebRequest CreateGetServiceGetRequest( string url )
		{
            ...
            if (!String.IsNullOrEmpty(this.GetNLogin()) && !String.IsNullOrEmpty(this.GetNPassword()))
            {
                request.Credentials = new NetworkCredential(this.GetNLogin(), this.GetNPassword());
            }
        ...
        private string GetNLogin()
        {
            return this._config.NLogin;
        }

        private string GetNPassword()
        {
            return this._config.NPassword;
        }
        		
		private void LogUpdateInfo( string shopName, string url, HttpStatusCode statusCode, string jsonContent )
		{
			ReverbLogger.Log.Trace( "[Reverb]\tPUT/POST call for shop '{0}' and url '{1}' has been completed with code '{2}'.\n{3}", shopName, url, statusCode, jsonContent );
		}
